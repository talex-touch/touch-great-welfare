...
function serializeNotificationProviderConfig(config: Awaited<ReturnType<typeof getEffectiveNotificationProviderConfig>>) {
  ...
  const smtpConfigured = !!(config.smtpEnabled && config.smtpHost && config.smtpPassword && config.smtpFromEmail)
  return {
    configured: {
      email: !!(config.resendApiKey && config.resendFromEmail) || feishuMailConfigured || smtpConfigured,
...
function applicationNotification(application: WelfareApplication, event: NotificationEvent) {
  ...
  const isPendingAllocation = application.status === 'pending_allocation'
  const isDelivered = application.status === 'delivered'
  return {
    title: isRejected ? `${typeName} 申请已退回` : isDelivered ? `${typeName} 资源已发放` : isPendingAllocation ? `${typeName} 申请待分配资源` : `${typeName} 申请已答复`,
    body: application.answer || (isRejected ? `你的 ${typeName} 申请已退回，已按退回规则处理积分。` : isDelivered ? `你的 ${typeName} 申请资源已发放，请进入申请详情查看。` : isPendingAllocation ? `你的 ${typeName} 申请已通过，等待管理员分配资源。` : `你的 ${typeName} 申请已通过并完成答复。`),
    data: {
